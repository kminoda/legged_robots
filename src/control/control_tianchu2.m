function u = control_tianchu2(t,q,dq)%(t, q, dq, q0, dq0, step_number)
...
% kd1 = 500; kd2 = 100;
% kd3 = 50;
% u1 = kp1*(q3-q1+pi/36) + kd1*(dq3-dq1) + kd3*(dq3-dq2);
% u2 = kp2*(q3-q2-5*pi/36) + kd2*(dq3-dq2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% kp1 = 50; kp2 = 50;
% kd1 = 5; kd2 = 0;
% kd3 = 5;
% u1 = kp1*(pi/24-q1) + kd1*(-dq1);
% u2 = kp2*(-pi/24-q2) + kd2*(-dq2);
kp1 = 50; kp2 = 100;
kd1 = 50; kd2 = 10;
kd3 = 50;
u1 = kp1*(q3-q1+ 4*pi/36) + kd1*(dq3-dq1) + kd3*(dq3-dq2) + 100*(q3+pi/36);
u2 = kp2*(q3-q2- 2*pi/36) + kd2*(dq3-dq2) + 10*(q3-pi/36);
u = [u1;u2];
u = saturate(u);
global historyU1;
global historyU2;

historyU1 = [historyU1, u(1)];
historyU2 = [historyU2, u(2)];
%% heuristic PD 3 (sln2)
% % Please use this initial condition: solve_eqns([pi/24; -pi/24; 0], [pi/12;-pi/12;pi/12], 20)
% % q1d = pi/6; dq1d = 0;
% % q2d = -pi/6; dq2d = 0;
% % q3d = 0; dq3d = 0;
% kp1 = 50; kp2 = 100;
% kd1 = 50; kd2 = 10;
% u1 = kp1*(q3-q1+pi/36) + kd1*(dq3-dq1) + 50*(dq3-dq2);
% u2 = kp2*(q3-q2-5*pi/36) + kd2*(dq3-dq2);
% u = [u1;u2]
%%
%document:
%sln1 = solve_eqns([pi/24; -pi/24; 0], [pi/12;-pi/12;pi/12], 20)
%% heuristic PD 4 (sln3)
% % Please use this initial condition: solve_eqns([pi/24; -pi/24; 0], [pi/12;-pi/12;pi/12], 20)
% kp1 = 50; kp2 = 100;
% kd1 = 50; kd2 = 10;
% u1 = kp1*(q3-q1 + 0 ) + kd1*(dq3-dq1) + 10*(dq3-dq2);
% u2 = kp2*(q3-q2-6*pi/36) + kd2*(dq3-dq2) - 50*(dq3+abs(dq3));
% u = [u1;u2]
end
