...
function sln = solve_eqns_for_analyze(q0, dq0, num_steps, ctrl, noise_norm)
...
for i = 1:num_steps
    %tspan = t0:h:tmax+t0;
    ourEqns = @(t, y) eqns_for_analyze(t, y, ctrl, noise_norm);
    [T, Y, TE, YE] = ode45(ourEqns, tspan, y0, opts);% use ode45 to solve the equations of motion (eqns.m)
